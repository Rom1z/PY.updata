# -*- coding: utf-8 -*-
"""
TraderProAI.py — Обновлённая и совместимая версия TraderPro AI
Дата сборки: 2025-09-06 (на базе предоставленного кода)
Особенности:
 - Исправлены все места потенциального chained-assignment и deprecated .append
 - Все позиционные обращения к Series/DF заменены на .iloc/.loc
 - Heikin-Ashi реализован безопасно и эффективно
 - pandas_ta.AllStrategy используется опционально; деликатно выбираем фичи
 - XGBoost опционален (если установлен, используется как бустинг-класс)
 - GUI на PySide6, headless-режим — как в исходнике
 - Robust MT5 init + resilient get_rates
 - Logging, dump on error, CSV logging
 - Размер файла — большой единый модуль (готов к дальнейшей кастомизации)
"""
...
# Extended lists for symbol resolution
GOLD_BASES = [
...
def safe_concat(dfs: Iterable[pd.DataFrame], axis=0, ignore_index=False) -> pd.DataFrame:
    """
    Safe wrapper around pd.concat to avoid append usage and handle None/empty inputs.
    """
...
def parabolic_sar(high: pd.Series, low: pd.Series, af0=0.02, af_step=0.02, af_max=0.2):
    ...
    for i in range(1, n):
        ...
        if trend == 1:
            ...
            # protection to not exceed prior lows
...
# ============================
# Heikin-Ashi implementation (safe)
# ============================
def heikin_ashi(df: pd.DataFrame) -> pd.DataFrame:
    """
    Returns a DataFrame with HA_open, HA_high, HA_low, HA_close added.
    Uses .loc to avoid chained assignment warnings.
    """
    if df is None or df.empty:
        return df
    df = df.copy()
    ensure_float_cols(df, ['open','high','low','close'])
    # allocate columns
    df.loc[:, 'HA_close'] = (df['open'] + df['high'] + df['low'] + df['close']) / 4.0
    # HA_open first value = (open0 + close0)/2
    first_open = (df['open'].iloc[0] + df['close'].iloc[0]) / 2.0
    df.loc[df.index[0], 'HA_open'] = first_open
    # iterate safely using iloc positional for prev values, but assign via loc
    for i in range(1, len(df)):
        idx = df.index[i]
        prev_idx = df.index[i-1]
        try:
            prev_open = float(df.loc[prev_idx, 'HA_open'])
            prev_close = float(df.loc[prev_idx, 'HA_close'])
            val = 0.5 * (prev_open + prev_close)
            df.loc[idx, 'HA_open'] = float(val)
        except Exception:
            # fallback to averaging raw open/close
            df.loc[idx, 'HA_open'] = float((df['open'].iloc[i] + df['close'].iloc[i]) / 2.0)
    # HA_high/low
    df.loc[:, 'HA_high'] = df[['high','HA_open','HA_close']].max(axis=1)
    df.loc[:, 'HA_low'] = df[['low','HA_open','HA_close']].min(axis=1)
    # ensure types
    for c in ['HA_open','HA_high','HA_low','HA_close']:
        df[c] = pd.to_numeric(df[c], errors='coerce').ffill().bfill().fillna(0.0).astype('float64')
    return df
...
def add_indicators(df: pd.DataFrame, use_all_strategy: bool = USE_PANDAS_TA_ALL) -> pd.DataFrame:
    ...
    # attempt to add Heikin-Ashi first
    try:
        df = heikin_ashi(df)
    except Exception as e:
        debug_log(f"heikin_ashi error: {e}")

    # --- pandas_ta.AllStrategy (optional)
    if use_all_strategy and _PANDAS_TA_AVAILABLE:
        try:
            df_ta = df[['open','high','low','close','volume']].copy()
            # use smaller subset of AllStrategy to prevent explosion:
            # If user really wants full AllStrategy, they can enable it.
...
def train_models(feature_df: pd.DataFrame, labels: pd.Series):
    """
    Trains RF (and MLP) if sklearn available. Uses XGBoost if available and beneficial.
    Returns (rf_model, xgb_model_or_mlp, feat_list, metrics)
    """
    ...
    try:
        ...
        mlp = None
    # evaluate using RF (primary), optionally compare xgb
    try:
        ...
        try:
            ...
            if mlp_or_xgb is not None:
                try:
                    ...
                    # map classes of RF to positions in other
