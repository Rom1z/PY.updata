# -*- coding: utf-8 -*-
"""
TraderProAI.py — Обновлённая совместимая версия TraderPro AI
Дата сборки: 2025-09-06 (на базе предоставленного кода)
Цели:
 - устранить chained-assignment и deprecated .append()
 - использовать .loc/.iloc для позиционной/меточной индексации
 - безопасная Heikin-Ashi реализация
 - опциональная поддержка pandas_ta.AllStrategy и XGBoost
 - GUI на PySide6 + headless fallback
 - логирование, дамп DF при ошибках
"""
...
def heikin_ashi(df: pd.DataFrame) -> pd.DataFrame:
    """
    Build Heikin-Ashi candles without chained assignment.
    Returns df with HA_open, HA_high, HA_low, HA_close.
    """
    if df is None or df.empty:
        return df
    ha = df.copy()
...
def add_indicators(df: pd.DataFrame, use_all_strategy: bool = USE_PANDAS_TA_ALL) -> pd.DataFrame:
    ...
    try:
        ha = heikin_ashi(df)
        # integrate HA columns (avoid duplicate 'close' override)
    ...
    # optional pandas_ta.AllStrategy
