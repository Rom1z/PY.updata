# TraderProAI_full.py
# Объединённая версия: оригинал из 11.txt + расширения (NewsWatcherV2, OrderBookWatcher, MarketMakerEstimator,
# expert90 (x2), ensemble aggregator и интеграция в analyze()).
#
# Сохраните как TraderProAI_full.py и запустите.
#
# Зависимости:
#   pip install pandas numpy PySide6 matplotlib scikit-learn xgboost (опционально) MetaTrader5 (если используете MT5)
#
# Внимание:
#   Тщательно протестируй на демо; не существует 100% сигнала — этот код повышает качество сигналов и добавляет защиту,
#   но требует валидации, бэктеста и мониторинга деградации.
...
# Utilities
# ============================
def safe_concat(dfs: Iterable[pd.DataFrame], axis=0, ignore_index=False) -> pd.DataFrame:
    parts = [d for d in dfs if d is not None and not (isinstance(d, pd.DataFrame) and d.empty)]
    if not parts:
        return pd.DataFrame()
    return pd.concat(parts, axis=axis, ignore_index=ignore_index)
...
# ============================
# Heikin-Ashi safe implementation
# ============================
def heikin_ashi(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return df
    ha = df.copy()
    ensure_float_cols(ha, ['open', 'high', 'low', 'close'])
    ha.loc[:, 'HA_close'] = (ha['open'] + ha['high'] + ha['low'] + ha['close']) / 4.0
    ha.loc[:, 'HA_open'] = np.nan
    try:
        ha.loc[ha.index[0], 'HA_open'] = (ha.loc[ha.index[0], 'open'] + ha.loc[ha.index[0], 'close']) / 2.0
    except Exception:
        ha.loc[ha.index[0], 'HA_open'] = (ha['open'].iloc[0] + ha['close'].iloc[0]) / 2.0
    for i in range(1, len(ha)):
        idx = ha.index[i]
        prev_idx = ha.index[i - 1]
        try:
            prev_open = float(ha.loc[prev_idx, 'HA_open'])
            prev_close = float(ha.loc[prev_idx, 'HA_close'])
            ha.loc[idx, 'HA_open'] = 0.5 * (prev_open + prev_close)
        except Exception:
            ha.loc[idx, 'HA_open'] = 0.5 * (ha.loc[idx, 'open'] + ha.loc[idx, 'close'])
    ha.loc[:, 'HA_high'] = ha[['high', 'HA_open', 'HA_close']].max(axis=1)
    ha.loc[:, 'HA_low'] = ha[['low', 'HA_open', 'HA_close']].min(axis=1)
    for c in ['HA_open', 'HA_high', 'HA_low', 'HA_close']:
        ha.loc[:, c] = pd.to_numeric(ha.loc[:, c], errors='coerce').ffill().bfill().fillna(0.0).astype('float64')
    return ha
...
def add_indicators(df: pd.DataFrame, use_all_strategy: bool = USE_PANDAS_TA_ALL) -> pd.DataFrame:
    ...
    # ✅ авто-отключение pandas_ta, если модуль не найден
    if use_all_strategy and not _PANDAS_TA_AVAILABLE:
        debug_log("pandas_ta не найден — AllStrategy отключен автоматически")
        use_all_strategy = False
    ...
    try:
        ...
        for col in ['HA_open','HA_high','HA_low','HA_close']:
            if col in ha.columns:
                df.loc[:, col] = ha.loc[:, col]
    except Exception as e:
        debug_log(f"heikin_ashi error: {e}")

    # AllStrategy из pandas_ta (только если доступна)
    ...
    # дальше твой код индикаторов (EMA, RSI, MACD, ATR и т.д.)
    # ...
    return df

    # EMAs
    try:
        df.loc[:, 'ema5'] = close.ewm(span=5, adjust=False).mean()
        df.loc[:, 'ema8'] = close.ewm(span=8, adjust=False).mean()
        df.loc[:, 'ema13'] = close.ewm(span=13, adjust=False).mean()
        df.loc[:, 'ema21'] = close.ewm(span=21, adjust=False).mean()
        df.loc[:, 'ema50'] = close.ewm(span=50, adjust=False).mean()
        df.loc[:, 'ema100'] = close.ewm(span=100, adjust=False).mean()
    except Exception as e:
    ...
    try:
        ...
        rs = up / (down + 1e-12)
        df.loc[:, 'rsi14'] = (100 - (100 / (1 + rs))).fillna(50.0)
    except Exception as e:
    ...
    try:
        ...
        df.loc[:, 'macd_sig'] = signal
        df.loc[:, 'macd_hist'] = macd_line - signal
    except Exception as e:
    ...
    try:
        ...
        df.loc[:, 'bb_mid'] = ma
        df.loc[:, 'bb_upper'] = ma + 2 * sd
        df.loc[:, 'bb_lower'] = ma - 2 * sd
    except Exception as e:
    ...
    try:
        ...
        df.loc[:, 'atr14'] = tr.rolling(ATR_PERIOD).mean().ffill().fillna(0.0)
    except Exception as e:
    ...
    try:
        ...
        df.loc[:, 'vwap'] = (c_vtp / c_vol).ffill().bfill().fillna(df['close']).astype('float64')
    except Exception as e:
    ...
    # Donchian
    try:
        df.loc[:, 'donch_high'] = df['high'].rolling(20).max().bfill().astype('float64')
        df.loc[:, 'donch_low'] = df['low'].rolling(20).min().bfill().astype('float64')
    except Exception as e:
    ...
    try:
        ...
        df.loc[:, 'adl'] = (clv * df['volume']).cumsum()
    except Exception as e:
        debug_log(f"ADL error: {e}")

    # PSAR
    try:
        df.loc[:, 'psar'] = parabolic_sar(df['high'], df['low'])
    except Exception as e:
    ...
    try:
        if 'bb_mid' in df.columns:
            df.loc[:, 'price_vs_bbmid'] = ((df['close'] - df['bb_mid']) / (df['bb_mid'] + 1e-9)).astype('float64')
    except Exception:
        pass
...
def train_models(feature_df: pd.DataFrame, labels: pd.Series):
    ...
    try:
        if _XGBOOST_AVAILABLE:
            try:
                xgb_clf = xgb.XGBClassifier(use_label_encoder=False, eval_metric='mlogloss', n_estimators=150, random_state=RANDOM_STATE)
                xgb_clf.fit(X_train, y_train)
                xgb_model = xgb_clf
            except Exception as e:
                debug_log(f"xgboost fit error: {e}")
                xgb_model = None
    except Exception:
        xgb_model = None
    try:
    ...
    try:
        ...
        metrics = {
            'accuracy': float(accuracy_score(y_test, y_pred)) if accuracy_score is not None else 0.0,
            'precision': float(precision_score(y_test, y_pred, average='macro', zero_division=0)) if precision_score is not None else 0.0,
            'recall': float(recall_score(y_test, y_pred, average='macro', zero_division=0)) if recall_score is not None else 0.0,
            'test_samples': int(len(y_test))
        }
    except Exception as e:
        debug_log(f"model evaluation error: {e}")
    ...
    if xgb_model is not None:
        try:
            ...
            metrics['xgb_feature_importances'] = {k: float(v) for k,v in zip(feats, xgb_imp) if v is not None}
        except Exception:
            pass
    return rf, (xgb_model if xgb_model is not None else mlp), feats, metrics
...
class NewsWatcherV2:
    """
    Расширенный news watcher: ingest_text + poll_newsapi (шаблон).
    Для продакшна: подключить NewsAPI/Polygon/биржевые ленты или RSS.
    """
    def __init__(self, newsapi_key: Optional[str]=None):
        self.lock = threading.Lock()
        self.score = 0.0
        self.last_text = ""
        self.history = []  # (ts, score, text)
        self.api_key = newsapi_key
        self.custom_keywords = {
            'bull': ['dovish','cut','risk on','weak dollar','qe','stimulus','beat','outperform','growth'],
            'bear': ['hawkish','hike','strong dollar','cpi','surprise inflation','rate hike','tightening','miss','downgrade']
        }

    def ingest_text(self, text: str):
        t = (text or "").lower()
        s = 0
        for w in self.custom_keywords['bull']:
            if w in t: s += 1
        for w in self.custom_keywords['bear']:
            if w in t: s -= 1
        s = max(-5, min(5, s))
        with self.lock:
            self.score = float(s) / 5.0
            self.last_text = text
            self.history.append((time.time(), self.score, text))

    def poll_newsapi(self, q='economy OR gold OR usd'):
        if not self.api_key:
            return
        import requests
        try:
            url = f"https://newsapi.org/v2/everything?q={q}&language=en&pageSize=20&apiKey={self.api_key}"
            ...
            for art in js.get('articles', []):
                ...
                title = title.strip()
                self.ingest_text(title)
                s_total += self.score
                cnt += 1
            if cnt > 0:
                avg = s_total / cnt
                with self.lock:
                    self.score = 0.5 * self.score + 0.5 * avg
        except Exception as e:
            debug_log(f"NewsWatcherV2 poll error: {e}")

    def get_sentiment(self) -> float:
        with self.lock:
            return float(self.score)

# instantiate with no key (user can set)
NEWS_V2 = NewsWatcherV2()
...
class Result:
    ...
    reasons: List[str]
    # extended:
    bull_strength_pct: Optional[int] = None
    bear_strength_pct: Optional[int] = None
    mm_prob_stop_hunt: Optional[float] = None
    expert_norms: Optional[Dict[str, float]] = None
...
class OrderBookWatcher:
    """
    OrderBook watcher template.
    If no WS available, use start_dummy_from_mt5_ticks as proxy.
    """
    def __init__(self, symbol: str, depth: int = 20):
        self.symbol = symbol
        self.depth = depth
        self.latest = {'bids': [], 'asks': []}
        self.imbalance = 0.0
        self.lock = threading.Lock()
        self.running = False
        self._thread = None
    ...
    def _process_snapshot(self, snap):
        bids = snap.get('bids', [])[:self.depth]
        asks = snap.get('asks', [])[:self.depth]
        ...
        with self.lock:
            self.latest = {'bids': bids, 'asks': asks}
            self.imbalance = imb

    def get_imbalance(self) -> float:
        with self.lock:
            return float(self.imbalance)

    def start_dummy_from_mt5_ticks(self, mt5_symbol: str, interval=0.5):
        def run():
            self.running = True
            samples = []
            while self.running:
                try:
                    ...
                    if tick:
                        samples.append((time.time(), getattr(tick, 'last', 0), getattr(tick, 'bid', 0), getattr(tick, 'ask', 0)))
                        cutoff = time.time() - 5.0
                        samples = [s for s in samples if s[0] >= cutoff]
                        bids = sum(1 for s in samples if s[2] >= s[3])
                        asks = len(samples) - bids
                        imb = (bids - asks) / max(1, len(samples))
                        with self.lock:
                            self.imbalance = imb
                    time.sleep(interval)
                except Exception:
...
# ============================
# --- NEW: MarketMakerEstimator ---
# ============================
class MarketMakerEstimator:
    def __init__(self, ob_watcher: Optional[OrderBookWatcher], volume_window_sec=10):
        self.ob = ob_watcher
        self.window = volume_window_sec
        self.last_spike_ts = 0

    def estimate(self, price_series: pd.Series, volume_series: pd.Series) -> Tuple[float, float, List[str]]:
        reasons = []
        mm_score = 0.0
        prob = 0.0
        try:
            imb = self.ob.get_imbalance() if self.ob is not None else 0.0
            mm_score += imb * 0.6
            reasons.append(f"Orderbook imbalance={imb:+.3f}")
            if len(volume_series) >= 20:
                recent = volume_series.iloc[-10:]
                med = volume_series.iloc[-60:-10].median() if len(volume_series) > 70 else volume_series.median()
                if med > 0 and recent.mean() > 3.0 * max(1e-9, med):
                    prob += 0.35
                    reasons.append("Обнаружен объёмный всплеск")
            if len(price_series) >= 6:
                p = price_series
                last = float(p.iloc[-1]); prev = float(p.iloc[-3])
                if abs((last - prev) / (prev+1e-9)) > 0.007:
                    prob += 0.2
                    reasons.append("Быстрый импульс — возможен sweep")
                    mm_score += 0.2 if last > prev else -0.2
            prob = min(1.0, prob)
            mm_score = max(-1.0, min(1.0, mm_score))
        except Exception as e:
            reasons.append(f"MM est error: {e}")
        return mm_score, prob, reasons

# ============================
# --- NEW: expert90_votes (2 variants) ---
# ============================
def expert90_votes_v1(df: pd.DataFrame, idx) -> Tuple[float, List[str]]:
    if df is None or idx not in df.index:
        return 0.0, ["Нет данных для Expert90_v1"]
    r = df.loc[idx]
    votes = 0.0; reasons = []
    try:
        if r.get('ema13',0) > r.get('ema50',0) and r.get('ema50',0) > r.get('ema100',0):
            votes += 1.5; reasons.append("Долгосрочный бычий тренд (13>50>100)")
        if r.get('rsi14',50) < 35:
            votes += 0.8; reasons.append("RSI сильная перепроданность")
        if r.get('macd_hist',0) > 0.5 * max(1.0, r.get('atr14',1)):
            votes += 0.9; reasons.append("MACD импульс сильный")
        if 'adl' in df.columns and not df['adl'].empty:
            if df['adl'].iloc[-1] > df['adl'].iloc[-20]:
                votes += 0.6; reasons.append("ADL растёт → покупательский поток")
            else:
                votes -= 0.5; reasons.append("ADL падает → продавц. поток")
    except Exception:
        pass
    return votes, reasons

def expert90_votes_v2(df: pd.DataFrame, idx) -> Tuple[float, List[str]]:
    if df is None or idx not in df.index:
        return 0.0, ["Нет данных для Expert90_v2"]
    r = df.loc[idx]
    votes = 0.0; reasons = []
    try:
        if r.get('close',0) > r.get('vwap',0):
            votes += 1.0; reasons.append("Цена > VWAP")
        if r.get('psar',0) < r.get('close',0):
            votes += 0.7; reasons.append("PSAR ниже цены")
        if r.get('price_vs_bbmid',0) > 0.02:
            votes -= 0.4; reasons.append("Ценовой разрыв от BB mid — вероятность отката")
    except Exception:
        pass
    return votes, reasons

# ============================
# --- NEW: Ensemble aggregator & bull/bear strength ---
# ============================
def compute_bull_bear_strength(ml_score: float, ind_score: float, news_score: float, cross_score: float, mm_score: float) -> Tuple[int,int]:
    w_ml, w_ind, w_news, w_cross, w_mm = 0.45, 0.2, 0.05, 0.15, 0.15
    combined = w_ml*ml_score + w_ind*ind_score + w_news*news_score + w_cross*cross_score + w_mm*mm_score
    bull = int(max(0, min(100, 50 + combined*50)))
    bear = 100 - bull
    return bull, bear

def ensemble_aggregate(probs: Dict[str,float], expert_scores: List[float], news_score: float, cross_score: float, mm_score: float) -> Tuple[str, float, int, int, List[str]]:
    ml_score = probs.get('BUY',0) - probs.get('SELL',0)
    ind_score = sum(expert_scores)/max(1,len(expert_scores))
    bull, bear = compute_bull_bear_strength(ml_score, ind_score, news_score, cross_score, mm_score)
    combined = 0.5*ml_score + 0.3*ind_score + 0.05*news_score + 0.1*cross_score + 0.05*mm_score
    agg_signal = "ДЕРЖАТЬ"
    if combined > 0.18: agg_signal = "ПОКУПАТЬ"
    if combined < -0.18: agg_signal = "ПРОДАВАТЬ"
    confidence = min(99.0, abs(combined)*120.0)
    reasons = [
        f"ML_score={ml_score:.3f}",
        f"Experts mean={ind_score:.3f}",
        f"News={news_score:.3f}",
        f"Cross={cross_score:.3f}",
        f"MM={mm_score:.3f}"
    ]
    return agg_signal, float(confidence), int(bull), int(bear), reasons

# ============================
# Analyze pipeline (modified to use ensemble and market maker)
# ============================
def analyze(symbol: str, timeframe_value: int, bars: int = DEFAULT_BARS) -> Result:
    ...
    if _SKLEARN_AVAILABLE:
        try:
            rf, mlp_or_xgb, feats, ml_metrics = train_models(df_ind, labels)
        except Exception as e:
    ...
    if rf is not None:
        try:
            ...
            if mlp_or_xgb is not None:
                try:
                    proba_other = mlp_or_xgb.predict_proba(X_latest)[0]
                    for i, c in enumerate(rf.classes_):
                        lab = 'SELL' if int(c)==0 else ('HOLD' if int(c)==1 else 'BUY')
                        probs[lab] = 0.5 * probs[lab] + 0.5 * float(proba_other[i])
                except Exception as e:
                    debug_log(f"mlp/xgb predict_proba error: {e}")
        except Exception as e:
            debug_log(f"model predict error: {e}")

    # experts
    expert_score, expert_reasons = expert60_votes(df_ind, latest_idx)
    e60_norm = max(-1.0, min(1.0, expert_score / 6.0))
    e90a_s, r90a = expert90_votes_v1(df_ind, latest_idx)
    e90b_s, r90b = expert90_votes_v2(df_ind, latest_idx)
    e90a_norm = max(-1.0, min(1.0, e90a_s / 6.0))
    e90b_norm = max(-1.0, min(1.0, e90b_s / 6.0))

    ind_component = (e60_norm + e90a_norm + e90b_norm) / 3.0

    # news
    news_component = max(-1.0, min(1.0, 0.5 * (NEWS.get_sentiment() + NEWS_V2.get_sentiment())))
    ...
    # market maker estimator
    obw = OrderBookWatcher(symbol)
    # optionally start MT5 tick proxy if MT5 available
    if _MT5_AVAILABLE:
        try:
            obw.start_dummy_from_mt5_ticks(symbol)
        except Exception:
            pass
    mm_est = MarketMakerEstimator(obw)
    mm_score, prob_stop_hunt, mm_reasons = mm_est.estimate(df_ind['close'], df_ind['volume'])

    # ensemble aggregator
    agg_signal, agg_confidence, bull_pct, bear_pct, ensemble_reasons = ensemble_aggregate(
        probs, [e60_norm, e90a_norm, e90b_norm], news_component, cross_component, mm_score
    )
    ...
    # adjust SL/lot if stop-hunt probable
    if prob_stop_hunt > 0.4 and agg_signal in ("ПОКУПАТЬ","ПРОДАВАТЬ"):
        try:
            if sl is not None:
                # expand SL distance by 30%
                if agg_signal == "ПОКУПАТЬ":
                    sl = price - (price - sl) * 1.3
                else:
                    sl = price + (sl - price) * 1.3
            if lots is not None:
                lots = round(max(0.01, lots * 0.7), 2)
        except Exception:
            pass

    reasons = [
        f"ML: BUY={probs['BUY']:.3f} HOLD={probs['HOLD']:.3f} SELL={probs['SELL']:.3f}",
        f"Expert60 score={expert_score:.2f} (norm={e60_norm:.2f})",
        f"Expert90a score={e90a_s:.2f} (norm={e90a_norm:.2f})",
        f"Expert90b score={e90b_s:.2f} (norm={e90b_norm:.2f})",
        f"News sentiment={news_component:.2f}",
        f"Cross-assets={cross_component:.2f} (вес {CROSS_WEIGHT:.2f})",
        f"MM score={mm_score:.2f} prob_stop_hunt={prob_stop_hunt:.2f}"
    ] + expert_reasons + r90a + r90b + mm_reasons + cross_reasons + ensemble_reasons
    ...
    res = Result(
        ...
        reasons=reasons,
        bull_strength_pct=bull_pct,
        bear_strength_pct=bear_pct,
        mm_prob_stop_hunt=prob_stop_hunt,
        expert_norms={'e60': e60_norm, 'e90a': e90a_norm, 'e90b': e90b_norm}
    )
    return res
...
def log_signal(res: Result):
    row = {
        ...
        'tp3': res.tp3,
        'lots': res.lots,
        'bull_pct': res.bull_strength_pct,
        'bear_pct': res.bear_strength_pct,
        'mm_prob_stop_hunt': res.mm_prob_stop_hunt
    }
    df = pd.DataFrame([row])
...
# ============================
# GUI (tidy) - same as in 11.txt (PySide6)
# ============================
APP_QSS = """
    ...
    class TraderProGUI(QWidget):
        ...
        def on_news(self):
            ...
            if txt:
                ...
                NEWS_V2.ingest_text(txt)
                self.out.append(f"Новости учтены. sentiment_old={NEWS.get_sentiment():+.2f} sentiment_v2={NEWS_V2.get_sentiment():+.2f}")
                self.newsEdit.clear()
        ...
        def _on_worker_finished(self, res: Result):
            try:
                ...
                lines.append(f"Сила быков: {res.bull_strength_pct}% | Сила медведей: {res.bear_strength_pct}% | prob_stop_hunt: {res.mm_prob_stop_hunt:.2f}")
                lines.append("\nМетрики модели:")
