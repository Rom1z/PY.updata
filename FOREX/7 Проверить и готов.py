...
Исправления: robust get_rates, расширенный find_symbol (включая все вариации золота),
замена deprecated .fillna(method='ffill') на .ffill(), улучшенная обработка ошибок MT5.
Дата: 2025-08-27 (исправлено)
"""
...
# Common currency pairs (no exhaustive list but main majors + popular suffixes)
CURRENCY_PAIRS = [
...
def _symbol_candidates(base: str):
    """
    Return a list of candidate names to try resolving in MT5.
    Includes many suffixes/prefixes that brokers may use.
    """
    ...
    # also try uppercase/lowercase normalization
    final = []
    for c in cand:
        final.append(c)
...
def mt5_init_or_raise(login: Optional[int] = None, password: Optional[str] = None, server: Optional[str] = None, force: bool = False):
    ...
    if not ok:
        ...
        # If incompatible versions -> give clear instruction
...
def find_symbol(base: str = BASE_SYMBOL) -> str:
    """
    Robust resolver: tries many candidate variations to find a symbol that exists in Market Watch.
    """
    if not _MT5_AVAILABLE:
    ...
    try:
        syms = mt5.symbols_get()
        if not syms:
            # We may have incompatible versions or terminal not ready
...
def _normalize_rates_df(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize dataframe returned by MT5 to columns: index=time, open, high, low, close, volume.
    Robust fallbacks for close: close -> last -> (bid+ask)/2 -> (open+high+low)/3
    """
...
def get_rates(symbol: str, timeframe: int, bars: int, verbose: bool = False) -> Optional[pd.DataFrame]:
    """
    Robust wrapper for mt5.copy_rates_from_pos & copy_rates_from.
    Multiple attempts + normalization + close fallbacks.
    """
    if not _MT5_AVAILABLE:
...
def add_indicators(df: pd.DataFrame, use_all_strategy: bool = USE_PANDAS_TA_ALL) -> pd.DataFrame:
    ...
    # ensure 'close' is preserved
    if 'close' not in df.columns:
        debug_log("add_indicators: unexpected - 'close' dropped; aborting")
        return df

    # feature limiting, variance/corr filtering (safe)
    try:
