...
Важно:
 ...
 - Установка зависимостей (пример):
    pip install PySide6 pandas numpy ccxt mplfinance openai
"""
import os
import sys
import math
...
def register_bulk_indicators():
    ...
    for a,b,s in macd_variants:
        ...
        def _macd_sig(df, a=a,b=b,s=s):
            return _ema(reg.get(f"macd_{a}_{b}_{s}_line")(df), s)
        @reg.register(f"macd_{a}_{b}_{s}_hist")
        def _macd_hist(df, a=a,b=b,s=s):
            return reg.get(f"macd_{a}_{b}_{s}_line")(df) - reg.get(f"macd_{a}_{b}_{s}_signal")(df)
    ...
    def _ich_span_a(df):
        return ((_ich_tenkan(df) + _ich_kijun(df))/2.0).shift(26)
...
def calc_trade_params(price: float, atr: float, risk_pct: float, account_balance: float, side: str,
    ...
    """
    try:
        if price is None or math.isnan(price) or atr is None or math.isnan(atr):
...
# ---------------------------
class AIAgent:
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-5-thinking-mini"):
...
2) Укажите ENTRY (число) или '-' для NEUTRAL, TP1/TP2/SL (числа) если даёте сигнал.
...
..."""
    ...
    def ask(self, symbol: str, timeframe: str, price: float, features: Dict[str, pd.Series], news: List[str]) -> SignalResult:
        ...
        for k in keys[-60:]:
            try:
                v = features[k].iloc[-1]
                summary_items.append(f"{k}={float(v):.4f}" if not np.isnan(v) else f"{k}=nan")
            except Exception:
        ...
        if self.api_key and openai:
            try:
                ...
                return self.parse_llm_response(text, price)
            except Exception as e:
    ...
    def parse_llm_response(self, text: str, price: float) -> SignalResult:
        ...
        if "recommendation:" in lower:
            ...
            if "short" in lower:
                side = "SHORT"
        # parse numbers for confidence and prices
        try:
            import re
            m = re.search(r"confidence:\s*([0-9]{1,3})", text, flags=re.IGNORECASE)
            if m:
                confidence = min(100.0, float(m.group(1)))/100.0
            ...
            for line in text.splitlines():
                ...
                if l.startswith("entry:"):
                    try:
                        ...
                        entry = None
                elif l.startswith("tp1:"):
                    try:
                        tp1 = float(line.split(":",1)[1].strip())
                    except Exception:
                        tp1 = None
                elif l.startswith("tp2:"):
                    try:
                        tp2 = float(line.split(":",1)[1].strip())
                    except Exception:
                        tp2 = None
                elif l.startswith("sl:"):
                    try:
                        sl = float(line.split(":",1)[1].strip())
                    except Exception:
                        sl = None
        except Exception:
            pass
        # if confidence < 0.55 then NEUTRAL by rule
        if confidence < 0.55:
            side = "NEUTRAL"
        reason = text
    ...
    def ai_fallback(self, features: Dict[str, pd.Series], price: float) -> SignalResult:
        # Simple heuristic-based fallback (keeps previous logic)
        last_vals = {k: (v.iloc[-1] if hasattr(v, 'iloc') and len(v)>0 else float('nan')) for k,v in features.items()}
        score = 0.0; reasons=[]
        try:
            if not np.isnan(last_vals.get('ema_50', np.nan)) and not np.isnan(last_vals.get('ema_200', np.nan)):
                if last_vals['ema_50'] > last_vals['ema_200']:
                    score += 0.06; reasons.append("Тренд восходящий (EMA50>EMA200)")
                else:
                    score -= 0.06; reasons.append("Тренд нисходящий (EMA50<EMA200)")
            macd = last_vals.get('macd_line', np.nan); macd_sig = last_vals.get('macd_signal', np.nan)
            if not np.isnan(macd) and not np.isnan(macd_sig):
                if macd > macd_sig:
                    score += 0.03; reasons.append("MACD положителен")
                else:
                    score -= 0.03; reasons.append("MACD отрицателен")
            rsi = last_vals.get('rsi_14', np.nan)
            if np.isnan(rsi):
                rsi = last_vals.get('rsi14', np.nan)
            if not np.isnan(rsi):
                if rsi < 30:
                    score += 0.03; reasons.append("RSI перепродан")
                elif rsi > 70:
                    score -= 0.03; reasons.append("RSI перекуплен")
            if not np.isnan(last_vals.get('obv', np.nan)):
                score += 0.005
        except Exception:
            pass
        ...
        atr = float(last_vals.get('atr14', 0.0) if not np.isnan(last_vals.get('atr14', np.nan)) else 0.0)
        tp1 = tp2 = tp3 = sl = None
        pos_size = None
        if entry:
            entry, tp1, tp2, tp3, sl, pos_size = calc_trade_params(entry, atr, 1.0, 10000.0, side)
...
class FetchWorker(QThread):
    ...
    def run(self):
        try:
            ...
            wanted = [k for k in ['ema_50','ema_200','macd_line','macd_signal','macd_hist','rsi_14','rsi14','stoch_k','stoch_d','atr14','bb_upper','bb_lower','vwap','obv','mfi','supertrend','donchian_up','donchian_dn','psar'] if k in df.columns]
            features_for_ai = {k: df[k] for k in wanted}
            ...
            # if AI didn't calculate position size -> compute using UI defaults later
...
class App(QWidget):
    def __init__(self):
        ...
        self.setGeometry(40, 40, 1600, 980)
    ...
    def apply_style(self):
        self.setStyleSheet("""
            QWidget { background-color: #0f1320; color: #E6E6E6; font-family: Segoe UI, Roboto, Arial; font-size: 11pt; }
            QGroupBox { border: 1px solid #2a2f45; border-radius: 12px; margin-top: 10px; padding: 8px; }
            QPushButton { background: #1f2540; border-radius: 8px; padding: 8px 10px; }
            QPushButton:hover { background: #2a325e; }
            QLineEdit, QComboBox, QSpinBox, QDoubleSpinBox { background: #151a2e; border: 1px solid #2a2f45; border-radius: 8px; padding: 6px; }
            QTabBar::tab { background: #151a2e; padding: 8px 12px; border-top-left-radius: 8px; border-top-right-radius: 8px; }
            QTabBar::tab:selected { background: #1d223a; }
            QTextEdit { background: #0f1320; border: 1px solid #2a2f45; border-radius: 8px; }
            QLabel { padding: 2px; }
        """)
    ...
    def on_fetched(self, df: pd.DataFrame, sig: SignalResult, symbol: str, timeframe: str):
        ...
        if 'atr14' in df.columns:
            last_atr = float(df['atr14'].iloc[-1]) if not np.isnan(df['atr14'].iloc[-1]) else None
        # if AI produced entry but no pos_size -> compute
        if sig.side in ("LONG","SHORT"):
            entry = sig.entry if sig.entry else float(df['close'].iloc[-1])
            # compute TP/SL if missing
            e,tp1,tp2,tp3,sl,pos = calc_trade_params(entry, last_atr if last_atr else 0.0, risk_pct, account_balance, sig.side, atr_sl_multiplier=atr_mul)
            # override missing with computed
            sig.entry = sig.entry or e
            sig.tp1 = sig.tp1 or tp1
            sig.tp2 = sig.tp2 or tp2
            sig.tp3 = sig.tp3 or tp3
            sig.sl = sig.sl or sl
            sig.position_size = sig.position_size or pos
        else:
        ...
        self.lbl_entry.setText(f"{sig.entry:.8f}" if sig.entry else "—")
        self.lbl_tp1.setText(f"{sig.tp1:.8f}" if sig.tp1 else "—")
        self.lbl_tp2.setText(f"{sig.tp2:.8f}" if sig.tp2 else "—")
        self.lbl_tp3.setText(f"{sig.tp3:.8f}" if sig.tp3 else "—")
        self.lbl_sl.setText(f"{sig.sl:.8f}" if sig.sl else "—")
        self.lbl_pos_size.setText(f"{sig.position_size:.6f}" if sig.position_size else "—")
        ...
        plot_df = None
        try:
            if self.ind_checkboxes.get('Heikin-Ashi') and self.ind_checkboxes['Heikin-Ashi'].isChecked():
            ...
            style = mpf.make_mpf_style(base_mpf_style='yahoo') if mpf else None
            try:
                mpf.plot(
                ...
                p = plot_df.tail(500).rename(columns={'Open':'open','High':'high','Low':'low','Close':'close','Volume':'volume'})
                mpf.plot(p, type='candle', ax=ax_price, volume=ax_vol, style=style, show_nontrading=True, warn_too_much_data=10000)
        except Exception as e:
            print("Plot failed:", e)
        ...
        try:
            ...
            if self.ind_checkboxes.get('SuperTrend') and self.ind_checkboxes['SuperTrend'].isChecked() and 'supertrend' in df.columns:
        ...
        try:
            if self.ind_checkboxes.get('Liquidity Pools') and self.ind_checkboxes['Liquidity Pools'].isChecked():
                ...
                if not np.isnan(hi): ax_price.axhline(float(hi), linestyle='--', linewidth=0.6, alpha=0.6)
                if not np.isnan(lo): ax_price.axhline(float(lo), linestyle='--', linewidth=0.6, alpha=0.6)
        except Exception:
            pass
